...
contract NFTBankImplementation is NFTBankStorageStructure {
    ...
    function claimNFT() external {
        require(!isLocked, "0670 pool is locked");
        ...
        require(_winnerNFTWithID.isWinner, "0671 only winners can purchase");
        ...
        if (!_winnerNFTWithID.didNFTwithdrawn) {
            ...
            if (nftType == erc721) {
                ...
                _transferERC721NFTs(
                    nftToken,
                    _msgSender(),
                    _winnerNFTWithID.nftID
                );
            }
        }
    }

    function _withdrawNFT(address _winner) internal {
    ...
    function updateNFTType(uint256 _nftType) public onlyPoolCreator {
        require(_nftType == erc721, "0302 only 721 ERCs");
    ...
    // TODO: waht is the exact number of winners
    function setWinners(
        address[] calldata _addrArray,
        uint256[] calldata _idArray
    ...
    function withdrawStuckERC721NFTs(
        address _stuckToken,
        address _receiver,
        uint256 _id
    ) external onlyPoolCreator {
