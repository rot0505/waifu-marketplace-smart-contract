...
describe('ERC1155 NFT Marketplace Smart Contract', () => {
  let Custom1155Factory, Custom1155RoyaltyFactory, AddressesFactory, ERC1155SaleFactory, ERC1155AuctionFactory, ProxyFactory, TokenFactory, ArrayLibraryFactory
  let custom1155, custom1155Royalty, addresses, erc1155Sale, erc1155Auction, proxy, token, arrayLibrary
  let accounts
  ...
  beforeEach(async () => {
    ...
    token = await TokenFactory.connect(accounts[3]).deploy()
    ...
    await (await erc1155Sale.connect(accounts[0]).setTokenContractAddr(token.address)).wait()
    await (await erc1155Auction.connect(accounts[0]).setTokenContractAddr(token.address)).wait()
