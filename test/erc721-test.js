...
describe('ERC721 NFT Marketplace Smart Contract', () => {
  let Custom721Factory, Custom721RoyaltyFactory, AddressesFactory, ERC721SaleFactory, ERC721AuctionFactory, ProxyFactory, ArrayLibraryFactory, TokenFactory
  let custom721, custom721Royalty, addresses, erc721Sale, erc721Auction, proxy, arrayLibrary, token
  let accounts
  ...
  beforeEach(async () => {
    ...
    await (await erc721Sale.connect(accounts[0]).setTokenContractAddr(token.address)).wait()
    await (await erc721Auction.connect(accounts[0]).setTokenContractAddr(token.address)).wait()
