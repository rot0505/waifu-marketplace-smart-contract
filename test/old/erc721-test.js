...
describe('ERC721 NFT Marketplace Smart Contract', () => {
  let Custom721Factory, Custom721RoyaltyFactory, AddressesFactory, MarketplaceFactory, NewMarketplaceFactory, ProxyFactory, ArrayLibraryFactory, RoyaltyLibraryFactory, SparkTokenFactory
  let custom721, custom721Royalty, addresses, marketplace, newMarketplace, proxy, arrayLibrary, royaltyLibrary, sparkToken
  let accounts
  ...
  before(async () => {
    ...
    NewMarketplaceFactory = await ethers.getContractFactory('ERC721MarketplaceUpgrade', { libraries: { ArrayLibrary: arrayLibrary.address } })
  ...
  it('Create Auction, Create Bid and Cancel Auction with Verified ERC721 NFT Smart Contract', async () => {
    ...
    expect(await custom721.balanceOf(accounts[0].address)).to.equal(2)
    let prevBalance = await accounts[1].getBalance()
    let txRes = await (await marketplace.connect(accounts[1]).bid(custom721.address, 1, offerPrice1, { value: offerPrice1 })).wait()
    let afterBalance = await accounts[1].getBalance()
    expect(afterBalance.add(txRes.cumulativeGasUsed.mul(txRes.effectiveGasPrice))).to.equal(prevBalance.sub(offerPrice1))
    prevBalance = await accounts[2].getBalance()
    txRes = await (await marketplace.connect(accounts[2]).bid(custom721.address, 1, offerPrice2, { value: offerPrice2 })).wait()
    afterBalance = await accounts[2].getBalance()
  ...
  it('Create Sale and Purchase with Verified ERC721 Royalty NFT Smart Contract', async () => {
    ...
    await (await marketplace.connect(accounts[0]).createSale(custom721Royalty.address, 1, 1, nftPrice, nftPrice, 1000)).wait()
    expect(await custom721Royalty.balanceOf(accounts[0].address)).to.equal(2)
    expect(await custom721Royalty.ownerOf(1)).to.equal(marketplace.address)
    const saleInfo = await marketplace.getSale(custom721Royalty.address, 1)
    expect(saleInfo.sale.seller).to.equal(accounts[0].address)
    const currentPrice = await marketplace.getCurrentPrice(custom721Royalty.address, 1)
    expect(currentPrice.lte(nftPrice)).to.equal(true)
    expect(currentPrice.gte(nftEndPrice)).to.equal(true)
    const prevBalance = await accounts[0].getBalance()
    const prevBalanceRoyalty = await accounts[4].getBalance()
    await (await marketplace.connect(accounts[1]).buy(custom721Royalty.address, 1, {
      from: accounts[1].address,
      value: currentPrice
    })).wait()
  ...
  it('Create Sale, Create Offer and Accept Offer with Verified ERC721 Royalty NFT Smart Contract', async () => {
    ...
    await (await addresses.verify(custom721Royalty.address)).wait()
    expect(await custom721Royalty.balanceOf(accounts[0].address)).to.equal(3)
    await (await marketplace.connect(accounts[0]).createSale(custom721Royalty.address, 1, 1, nftPrice, nftPrice, 0)).wait()
    expect(await custom721Royalty.balanceOf(accounts[0].address)).to.equal(2)
    expect(await custom721Royalty.ownerOf(1)).to.equal(marketplace.address)
    let saleInfo = await marketplace.getSale(custom721Royalty.address, 1)
    expect(saleInfo.sale.seller).to.equal(accounts[0].address)
    expect(saleInfo.offers.length).to.equal(0)
    let prevBalance = await accounts[1].getBalance()
    let txRes = await (await marketplace.connect(accounts[1]).makeOffer(custom721Royalty.address, 1, offerPrice1, { from: accounts[1].address, value: offerPrice1 })).wait()
    let afterBalance = await accounts[1].getBalance()
    expect(prevBalance.sub(txRes.cumulativeGasUsed.mul(txRes.effectiveGasPrice))).to.equal(afterBalance.add(offerPrice1))
    prevBalance = await accounts[2].getBalance()
    txRes = await (await marketplace.connect(accounts[2]).makeOffer(custom721Royalty.address, 1, offerPrice2, { from: accounts[2].address, value: offerPrice2 })).wait()
    afterBalance = await accounts[2].getBalance()
    expect(prevBalance.sub(txRes.cumulativeGasUsed.mul(txRes.effectiveGasPrice))).to.equal(afterBalance.add(offerPrice2))
    saleInfo = await marketplace.getSale(custom721Royalty.address, 1)
    expect(saleInfo.sale.seller).to.equal(accounts[0].address)
    expect(saleInfo.offers.length).to.equal(2)
    expect(saleInfo.offers[0].offerer).to.equal(accounts[1].address)
    expect(saleInfo.offers[0].offerPrice).to.equal(offerPrice1)
    ...
    prevBalance = await accounts[0].getBalance()
    const prevBalanceRoyalty = await accounts[4].getBalance()
    txRes = await (await marketplace.connect(accounts[0]).acceptOffer(custom721Royalty.address, 1)).wait()
    afterBalance = await accounts[0].getBalance()
    const afterBalanceRoyalty = await accounts[4].getBalance()
  ...
  it('Create Auction, Create Bid and Accept Bid with Verified ERC721 Royalty NFT Smart Contract', async () => {
    ...
    await (await addresses.verify(custom721Royalty.address)).wait()
    expect(await custom721Royalty.balanceOf(accounts[0].address)).to.equal(3)
    await (await marketplace.connect(accounts[0]).createAuction(custom721Royalty.address, 1, 1)).wait()
    expect(await custom721Royalty.balanceOf(accounts[0].address)).to.equal(2)
    let prevBalance = await accounts[1].getBalance()
    let txRes = await (await marketplace.connect(accounts[1]).bid(custom721Royalty.address, 1, offerPrice1, { value: offerPrice1 })).wait()
    let afterBalance = await accounts[1].getBalance()
    expect(afterBalance.add(txRes.cumulativeGasUsed.mul(txRes.effectiveGasPrice))).to.equal(prevBalance.sub(offerPrice1))
    prevBalance = await accounts[2].getBalance()
    txRes = await (await marketplace.connect(accounts[2]).bid(custom721Royalty.address, 1, offerPrice2, { value: offerPrice2 })).wait()
    afterBalance = await accounts[2].getBalance()
    expect(afterBalance.add(txRes.cumulativeGasUsed.mul(txRes.effectiveGasPrice))).to.equal(prevBalance.sub(offerPrice2))
    prevBalance = await accounts[0].getBalance()
    const prevBalanceRoyalty = await accounts[4].getBalance()
    txRes = await (await marketplace.connect(accounts[0]).acceptBid(custom721Royalty.address, 1)).wait()
    afterBalance = await accounts[0].getBalance()
    const afterBalanceRoyalty = await accounts[4].getBalance()
  ...
  it('Upgrade ERC721 NFT Marketplace Smart Contract', async () => {
  ...
  it('Create Sale and Purchase with Upgraded ERC721 NFT Marketplace Smart Contract', async () => {
    newMarketplace = await upgrades.deployProxy(NewMarketplaceFactory, [], { unsafeAllow: ['external-library-linking'] })
    await newMarketplace.deployed()
    await (await newMarketplace.connect(accounts[0]).setSparkTokenContractAddr(sparkToken.address)).wait()
    proxy.connect(accounts[0]).setProxy(newMarketplace.address)
    ...
    await (await addresses.verify(custom721.address)).wait()
    expect(await custom721.balanceOf(accounts[0].address)).to.equal(3)
    await (await newMarketplace.connect(accounts[0]).createSale(custom721.address, 1, 1, nftPrice, nftEndPrice, 1000)).wait()
    expect(await custom721.balanceOf(accounts[0].address)).to.equal(2)
    expect(await custom721.ownerOf(1)).to.equal(newMarketplace.address)
    const saleInfo = await newMarketplace.getSale(custom721.address, 1)
    expect(saleInfo.sale.seller).to.equal(accounts[0].address)
    const currentPrice = await newMarketplace.getCurrentPrice(custom721.address, 1)
    expect(currentPrice.lte(nftPrice)).to.equal(true)
    expect(currentPrice.gte(nftEndPrice)).to.equal(true)
    const prevBalance = await accounts[0].getBalance()
    const buyRes = await (await newMarketplace.connect(accounts[1]).buy(custom721.address, 1, {
      from: accounts[1].address,
      value: currentPrice
    })).wait()
    expect(await custom721.balanceOf(accounts[1].address)).to.equal(1)
